...
public:
	...
	void insert(string str) const
	{
		...
		// Returns level (n) in tree and updates Node ptr (cur)
		unsigned int n = findPrefixEnd(str, cur);

		// Insert new nodes containing rest of str (if necessary)
		for (; n < str.length(); n++)
		{
			// Use of [] is not optimal, as we always want to insert, as [] first searches.
			cur->children[str[n]] = new Node(str[n], false, cur);
			// Insert requires an iterator or a pair (duplication of decleration):
			//cur->children.insert(std::pair<char, Node*>(str[n], new Node(str[n], false, cur)));
			// C++11
			//cur->children.emplace(str[n], new Node(str[n], false, cur));
