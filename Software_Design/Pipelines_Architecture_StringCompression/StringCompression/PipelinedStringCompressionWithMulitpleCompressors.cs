...
{
    internal class PipelinedStringCompressionWithMulitpleCompressors
    {
        ...
        public double Run()
        {
            ...
            Task compressionTask1 = Task.Run(() => CompressStringsStage(generatedStringQueue, compressedStringQueue1));
            Task compressionTask2 = Task.Run(() => CompressStringsStage(generatedStringQueue, compressedStringQueue2));
            Task compressionTask3 = Task.Run(() => CompressStringsStage(generatedStringQueue, compressedStringQueue3));
            Task compressionTask4 = Task.Run(() => CompressStringsStage(generatedStringQueue, compressedStringQueue4));
            Task updateCompressionStatsTask = Task.Run(() => UpdateCompressionStatsStage(new[] { compressedStringQueue1, compressedStringQueue2, compressedStringQueue3, compressedStringQueue4 }, ref _avgCompressionRatio));

            Task.WaitAll(generateStringsTask, compressionTask1, updateCompressionStatsTask);
        ...
        {
            int i = 0;
            CompressedStringInfo info = new CompressedStringInfo();
